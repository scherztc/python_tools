...
def plot_motion():
    ...
    ax[0].plot(sol.t, sol.y[0], label='Mass 1 Displacement (m)')
    ax[0].plot(sol.t, sol.y[2], label='Mass 2 Displacement (m)')
    ax[0].set_xlabel('Time (s)')
    ax[0].set_ylabel('Displacement (m)')
    ax[0].set_title('Coupled Harmonic Oscillators Motion')
    ax[0].legend()
    ax[0].grid()
    ...
    for i in range(0, len(sol.t), 10):
        ax[1].clear()
        ax[1].plot([0, 1], [sol.y[0, i], sol.y[2, i]], 'ro-', markersize=10)
        ax[1].set_xlim(-0.5, 1.5)
        ax[1].set_ylim(-2, 2)
        ax[1].set_title('Mass Positions Over Time')
