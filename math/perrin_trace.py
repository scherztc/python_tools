...
def display_trace_table(trace_table):
    ...
    tk.Label(trace_window, text="x").grid(row=0, column=1)
    tk.Label(trace_window, text="y").grid(row=0, column=2)
    tk.Label(trace_window, text="z").grid(row=0, column=3)
    
    for i, (w, x, y, z) in enumerate(trace_table, start=1):
        tk.Label(trace_window, text=str(w)).grid(row=i, column=0)
        tk.Label(trace_window, text=str(x)).grid(row=i, column=1)
        tk.Label(trace_window, text=str(y)).grid(row=i, column=2)
        tk.Label(trace_window, text=str(z)).grid(row=i, column=3)
