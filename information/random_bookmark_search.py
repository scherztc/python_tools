...
keyword = input("Enter a keyword to search for URLs (or press Enter to skip): ").strip().lower()

# Filter URLs based on the keyword
filtered_urls = [url for url in urls if keyword in url.lower()] if keyword else urls

# Sort URLs alphabetically
filtered_urls = sorted(filtered_urls)

# Write URLs to a text file and output the number of URLs
...
if filtered_urls:
    random_url = random.choice(filtered_urls)
